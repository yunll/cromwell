...
class VkWorkflowPathsSpec extends AnyFlatSpec with Matchers with BackendSpec {

  "WorkflowPaths" should "provide correct paths for a workflow" in {
    val wd = buildWdlWorkflowDescriptor(VkWorkflows.HelloWorld)
    val workflowPaths = VkWorkflowPaths(wd, VkTestConfig.backendConfig)
    val id = wd.id
  ...
  "WorkflowPaths" should "provide correct paths for a sub workflow" in {
    ...
    val workflowPaths = VkWorkflowPaths(subWd, VkTestConfig.backendConfig)
