...
class VkRuntimeAttributesSpec extends WordSpecLike with Matchers {
  ...
  private val mockConfigurationDescriptor = BackendConfigurationDescriptor(VkTestConfig.backendConfig, TestConfig.globalConfig)
  private val mockTesConfiguration = new VkConfiguration(mockConfigurationDescriptor)

  private def assertSuccess(runtimeAttributes: Map[String, WomValue],
                            expectedRuntimeAttributes: VkRuntimeAttributes,
  ...
  private val staticRuntimeAttributeDefinitions: Set[RuntimeAttributeDefinition] =
    VkRuntimeAttributes.runtimeAttributesBuilder(mockTesConfiguration.runtimeConfig).definitions.toSet


  private def toTesRuntimeAttributes(runtimeAttributes: Map[String, WomValue],
                                     workflowOptions: WorkflowOptions,
                                     tesConfiguration: VkConfiguration): VkRuntimeAttributes = {
    val runtimeAttributesBuilder = VkRuntimeAttributes.runtimeAttributesBuilder(tesConfiguration.runtimeConfig)
    ...
    VkRuntimeAttributes(validatedRuntimeAttributes, tesConfiguration.runtimeConfig
    )
  }
}
