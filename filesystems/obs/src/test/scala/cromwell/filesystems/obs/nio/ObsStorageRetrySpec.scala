...
class ObsStorageRetrySpec extends TestKitSuite with ObsNioUtilSpec {

  import ObsStorageRetrySpec._

  behavior of s"ObsStorageRetrySpec"
  ...
  it should "retry throw immediately when fatal error occours" in {
    ...
    an [FatalError] should be thrownBy ObsStorageRetry.fromTry(
      () => Try{
        ctx.doSth(f)
      },
      isFatal = isFatal,
  ...
  it should "retry at most max retry times" in {
    val needRetry = ObsStorageRetry.DEFAULT_MAX_RETRIES + 1
    val f = (x: Int) => {
      if (x < needRetry) {
        Failure(new RetryableError())
      } else {
    ...
    val ctx = new RetryContext()
    an [RetryableError] should be thrownBy ObsStorageRetry.fromTry(
      () => Try{
        ctx.doSth(f)
      },
      isFatal = isFatal,
    ...
    ctx.retried shouldEqual(ObsStorageRetry.DEFAULT_MAX_RETRIES)
  }
}
