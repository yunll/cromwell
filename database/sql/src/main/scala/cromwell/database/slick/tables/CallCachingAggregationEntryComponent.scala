...
//  }
  ...
  def insertPrefixHints(prefixHints: List[String]) = {
    val insertValues = prefixHints.map(s => s"""("$s%")""").mkString(",")
    val str = s"""INSERT INTO CALL_CACHING_PREFIX_HINTS(PREFIX_HINTS) VALUES $insertValues"""
...
//    println(str)
  ...
  def createTempPrefixHintTable() = {
    val str = "CREATE TEMPORARY TABLE CALL_CACHING_PREFIX_HINTS(PREFIX_HINTS varchar(255))"
...
//    println(str)
  ...
  def dropTempTablePrefixHints = {
    val str = """DROP TEMPORARY TABLE CALL_CACHING_PREFIX_HINTS"""
...
//    println(str)
  ...
  def existsQuery(baseAggregation: String) = {
    val str =
      s"""
        |select exists(
        |	select 1
        ...
        |				((x2.`ALLOW_RESULT_REUSE` and (x2.`CALL_CACHING_ENTRY_ID` = x3.`CALL_CACHING_ENTRY_ID`)) and
        |                     (x6.x4 = x2.`CALL_CACHING_ENTRY_ID`)))
        |                     and (x6.x5 like x7.`PREFIX_HINTS`)
        |);
      """.stripMargin
...
//  }
  ...
  def findCallCacheHitQuery(baseAggregation: String, inputFilesAggregation: Option[String], number: Int) = {
    val inputs = inputFilesAggregation.map(input => s"'$input'")getOrElse("null")
    ...
    val str =
      s"""
        ...
        |      )
        |   )
        |   and (x7.x6 like x8.`PREFIX_HINTS`)
        |   )
        |   limit $start, 1;
      """.stripMargin
